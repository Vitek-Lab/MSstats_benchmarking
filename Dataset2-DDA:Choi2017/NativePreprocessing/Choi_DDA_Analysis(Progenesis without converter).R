...
raw <- read.csv("C:/Users/tushi/OneDrive/Desktop/Data/DDA_choi_progenesis/Choi2017_DDA_Progenesis_input.csv", stringsAsFactors=F,skip=2) # the data file
annot <- read.csv('C:/Users/tushi/OneDrive/Desktop/Data/DDA_choi_progenesis/Choi2017_DDA_Progenesis_annotation.csv')
raw=raw[,1:30]
...
input_data = raw %>% select(Accession, Sequence, Charge, starts_with("JD"))
...
TP<-iprg_deqms_results %>% filter(grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow() #19
FP<-iprg_deqms_results %>% filter(!grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow() #350
...
FP<-iprg_msqrob_results %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & adjPval < .05) %>% nrow() #179
...
colnames(e_mdata)<-str_replace_all(colnames(e_mdata), '[-.]','_')
vec<-data.frame(colnames(e_mdata))

annot$Run=str_replace_all(annot$Run, '[-]','_')
...
mypepData <- as.pepData(e_data=e_mdata, f_data = f_mdata, e_meta=test,edata_cname = "Sequence", emeta_cname="Protein",fdata_cname = "Run", data_scale = "abundance",check.names=FALSE)
...
TP<-m %>% filter(grepl(paste(proteins, collapse="|"),Protein) & `p-value` < .05) %>% nrow() #22
FP<-m %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & `p-value` < .05) %>% nrow() #431
...
df.LFQ$Run<-str_replace_all(df.LFQ$Run, '[-.]','_')
...
TP<-df %>% filter(grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow()  #19
FP<-df %>% filter(!grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow()  #676
...
TP<-test %>% filter(grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #17
FP<-test %>% filter(!grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #335
