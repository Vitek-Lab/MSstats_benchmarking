...
# Reading data
##############################
raw <- read.csv(file="C:/Users/tushi/OneDrive/Desktop/Data/DDA_choi_skyline/Choi2017_DDA_Skyline_input.csv")
annotation <- read.csv('C:/Users/tushi/OneDrive/Desktop/Data/DDA_choi_skyline/Choi2017_DDA_Skyline_annotation.csv')
...
TP<-test.MSstats %>% filter(grepl(paste(proteins, collapse="|"),Protein) & adj.pvalue < .05) %>% nrow() #23
FP<-test.MSstats %>% filter(!grepl(paste(proteins, collapse="|"),Protein)& adj.pvalue < .05) %>% nrow() #6

##################################### DeqMS###################################################################
input_data = raw %>% select(ProteinName, PeptideSequence, PrecursorCharge, FileName, Area)


input_data$PSM = paste(input_data$PeptideSequence, input_data$PrecursorCharge, sep="_")
input_data = input_data %>% group_by(ProteinName, FileName, PSM) %>% summarise(Intensity=max(Area))
input_data = input_data %>% filter(ProteinName != "")
input_data = input_data %>% dplyr::rename(ProteinName=ProteinName, Run = FileName)
...
pep.count <- as.data.frame(raw )%>% select(ProteinName, PeptideSequence)
...
TP<-iprg_deqms_results %>% filter(grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow() #10
FP<-iprg_deqms_results %>% filter(!grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow() #35
...
TP<-iprg_msqrob_results %>% filter(grepl(paste(proteins, collapse="|"),Protein) & adjPval < .05) %>% nrow() #22
FP<-iprg_msqrob_results %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & adjPval < .05) %>% nrow() #12
...
e_mdata<-raw[,c('PeptideSequence','FileName','Area')]
e_mdata$Area=as.numeric(e_mdata$Area)
e_mdata<-e_mdata%>%group_by(PeptideSequence,FileName)%>% summarise(Intensity=sum(Area))
e_mdata<-e_mdata %>% pivot_wider(names_from = FileName, values_from = Intensity,values_fill = NA)

vec<-data.frame(c=colnames(e_mdata))
f_mdata<-annot[match(vec[2:13,],annot$Run),]

test<-raw[,c('PeptideSequence','ProteinName')]
...
mypepData <- as.pepData(e_data=e_mdata, f_data = f_mdata, e_meta=test,edata_cname = "PeptideSequence", emeta_cname="Protein",fdata_cname = "Run", data_scale = "abundance",check.names=FALSE)
...
TP<-m %>% filter(grepl(paste(proteins, collapse="|"),Protein) & `p-value` < .05) %>% nrow() #24
FP<-m %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & `p-value` < .05) %>% nrow() #245

#####################DEP######################################################################
df.LFQ = raw %>% select(ProteinName, FileName, Area)

df.LFQ = df.LFQ %>% group_by(ProteinName, FileName) %>% summarise(Intensity = max(Area, na.rm = TRUE))
df.LFQ$Intensity=as.numeric(df.LFQ$Intensity)
df.LFQ = as.data.frame(df.LFQ)
## Convert into Intensity format
df.LFQ = reshape(df.LFQ, idvar = "ProteinName", timevar = "FileName", direction = "wide")

data_unique <- make_unique(df.LFQ, "ProteinName", "ProteinName", delim = ";")



expdesign<-select(annotation,c('Run',"Condition","BioReplicate"))

colnames(expdesign)=c('label','condition','replicate')  
expdesign$replicate<-rep(1:12)
...
TP<-df %>% filter(grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow() #5
FP<-df %>% filter(!grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow() #98
...
df.LFQ = raw %>% select(ProteinName, FileName, Area)
df.LFQ$Area=as.numeric(df.LFQ$Area)
df.LFQ = df.LFQ %>% group_by(ProteinName, FileName) %>% summarise(Intensity = sum(Area, na.rm = TRUE))
...
# Results
TP<-test %>% filter(grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #15
FP<-test %>% filter(!grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #100
