...
TP<-iprg_deqms_results %>% filter(grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow() #167
FP<-iprg_deqms_results %>% filter(!grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow() #140
...
TP<-iprg_msqrob_results %>% filter(grepl(paste(proteins, collapse="|"),Protein) & adjPval < .05) %>% nrow() #154
FP<-iprg_msqrob_results %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & adjPval < .05) %>% nrow() #101
...
annot$Run.new<-str_replace(annot$Run, "[.]", "")
...
f_mdata<-annot[match(vec[2:16,],annot$Run.new),]
...
TP<-m %>% filter(grepl(paste(proteins, collapse="|"),Protein) & `p-value` < .05) %>% nrow() #154
FP<-m %>% filter(!grepl(paste(proteins, collapse="|"),Protein)& `p-value` < .05) %>% nrow() #170
...
expdesign$label=gsub('[.]', '', expdesign$label)
...
TP<-df %>% filter(grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow() #164
FP<-df %>% filter(!grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow() #304
...
TP<-test %>% filter(grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #152
FP<-test %>% filter(!grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #178
...
TP<- m%>%filter(grepl(paste(proteins, collapse="|"),protein_Id) & `p-value` < .05) %>% nrow()#169
FP<-m%>%filter(!grepl(paste(proteins, collapse="|"),protein_Id) & `p-value` < .05) %>% nrow()#179
...
TP<- m%>%filter(grepl(paste(proteins, collapse="|"),protein) & adj.pvalue < .05) %>% nrow()#167
FP<-m%>%filter(!grepl(paste(proteins, collapse="|"),protein) & adj.pvalue < .05) %>% nrow()#142
