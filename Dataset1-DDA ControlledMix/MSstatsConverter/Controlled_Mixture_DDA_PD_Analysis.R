...
quant <- PDtoMSstatsFormat(raw, 
                           annotation=annot,useUniquePeptide = TRUE,
...
FP<-test.MSstats %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & adj.pvalue < .05) %>% nrow() #205
...
expdesign$Run=gsub('[.]','',expdesign$Run)
...
FP<-m%>%filter(!grepl(paste(proteins, collapse="|"),protein_Id) & `p-value` < .05) %>% nrow()#56
...
TP<- m%>%filter(grepl(paste(proteins, collapse="|"),protein) & adj.pvalue < .05) %>% nrow()#190
FP<-m%>%filter(!grepl(paste(proteins, collapse="|"),protein) & adj.pvalue < .05) %>% nrow()#52
