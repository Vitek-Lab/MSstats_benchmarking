...
protein.matrix <- log2(as.matrix(df.LFQ.filter[1:15]))
...
#calculating TP , FP
TP<-iprg_deqms_results %>% filter(grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow()#199
FP<-iprg_deqms_results %>% filter(!grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow()#112
...
FP<-iprg_msqrob_results %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & adjPval < .05) %>% nrow() #131
...
TP<-m %>% filter(grepl(paste(proteins, collapse="|"),Protein) & `p-value` < .05) %>% nrow() #203
FP<-m %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & `p-value` < .05) %>% nrow() #1048
...
df.LFQ = as.data.frame(input_data) %>% select(ProteinName, Run, Intensity)
df.LFQ = df.LFQ %>% group_by(ProteinName, Run) %>% summarise(Intensity = max(Intensity, na.rm = TRUE))
df.LFQ = as.data.frame(df.LFQ)
## Convert into Intensity format
df.LFQ = reshape(df.LFQ, idvar = "ProteinName", timevar = "Run", direction = "wide")
...
TP<-df %>% filter(grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow() #194
FP<-df %>% filter(!grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow() #278
...
TP<-test %>% filter(grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #191
FP<-test %>% filter(!grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #214
...
TP<- m%>%filter(grepl(paste(proteins, collapse="|"),protein_Id) & `p-value` < .05) %>% nrow()#204
FP<-m%>%filter(!grepl(paste(proteins, collapse="|"),protein_Id) & `p-value` < .05) %>% nrow()#188
...
TP<- m%>%filter(grepl(paste(proteins, collapse="|"),protein) & adj.pvalue < .05) %>% nrow()#202
FP<-m%>%filter(!grepl(paste(proteins, collapse="|"),protein) & adj.pvalue < .05) %>% nrow()#111
