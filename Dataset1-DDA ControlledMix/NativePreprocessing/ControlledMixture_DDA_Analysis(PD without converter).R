...
input_data = raw %>% select(Protein.Group.Accessions, Sequence, Charge, Spectrum.File, Intensity)

input_data$PSM = paste(input_data$Sequence, input_data$Charge, sep="_")
input_data = input_data %>% group_by(Protein.Group.Accessions, Spectrum.File, PSM) %>% summarise(Intensity=max(Intensity))
input_data = input_data %>% filter(Protein.Group.Accessions != "")
input_data = input_data %>% dplyr::rename(ProteinName=Protein.Group.Accessions, Run = Spectrum.File)
...
pep.count <- as.data.frame(raw )%>% select(Protein.Group.Accessions, Sequence)
pep.count<-pep.count%>%group_by(Protein.Group.Accessions)%>%
...
rownames(pep.count) = pep.count$Protein.Group.Accessions
pep.count = pep.count %>% select(-Protein.Group.Accessions)

#protein.matrix[sapply(protein.matrix, is.infinite)] <- NA

## DEqMS analysis on LFQ data
protein.matrix <- na.omit(as.matrix(df.LFQ.filter[1:15]))
...
TP<-iprg_deqms_results %>% filter(grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow() #72
FP<-iprg_deqms_results %>% filter(!grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow() #24
...
TP<-iprg_msqrob_results %>% filter(grepl(paste(proteins, collapse="|"),Protein) & adjPval < .05) %>% nrow() #56
FP<-iprg_msqrob_results %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & adjPval < .05) %>% nrow() #22
...
e_mdata<-raw[,c('Sequence','Spectrum.File','Intensity')]
e_mdata<-e_mdata%>%group_by(Sequence,Spectrum.File)%>% summarise(Intensity=sum(Intensity))
e_mdata<-e_mdata %>% pivot_wider(names_from = Spectrum.File, values_from = Intensity,values_fill = NA)
...
test<-raw[,c('Sequence','Protein.Group.Accessions')]
test %>%group_by(Sequence) %>%
  fill(Protein.Group.Accessions, .direction = "downup") %>%
  ungroup %>%
  distinct
...
TP<-m %>% filter(grepl(paste(proteins, collapse="|"),Protein) & `p-value` < .05) %>% nrow()  #41
FP<-m %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & `p-value` < .05) %>% nrow() #133


################################## DEP ##########################################################################


df.LFQ = as.data.frame(raw) %>% select(Protein.Group.Accessions, Spectrum.File, Intensity)
df.LFQ = df.LFQ %>% group_by(Protein.Group.Accessions, Spectrum.File) %>% summarise(Intensity = sum(Intensity, na.rm = TRUE))
df.LFQ = as.data.frame(df.LFQ)
df.LFQ = df.LFQ %>% filter(Protein.Group.Accessions != "")
df.LFQ=df.LFQ%>%rename(ProteinName=Protein.Group.Accessions,Run=Spectrum.File)
...
diff <- DEP::test_diff(norm, type="all")
...
TP<-df %>% filter(grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow() #100
FP<-df %>% filter(!grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow() #77
...
df.LFQ = raw %>% select(Protein.Group.Accessions, Spectrum.File, Intensity)
df.LFQ = df.LFQ %>% filter(Protein.Group.Accessions != "")
df.LFQ = df.LFQ %>% group_by(Protein.Group.Accessions, Spectrum.File) %>% summarise(Intensity = sum(Intensity, na.rm = TRUE))
df.LFQ = as.data.frame(df.LFQ)
df.LFQ=df.LFQ%>%rename(ProteinName=Protein.Group.Accessions,Run=Spectrum.File)
...
TP<-test %>% filter(grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #48
FP<-test %>% filter(!grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #25
...
TP<- m%>%filter(grepl(paste(proteins, collapse="|"),protein_Id) & `p-value` < .05) %>% nrow()#61
FP<-m%>%filter(!grepl(paste(proteins, collapse="|"),protein_Id) & `p-value` < .05) %>% nrow()#29
...
protein.matrix <- na.omit(as.matrix(df.LFQ[1:15]))
...
TP<- m%>%filter(grepl(paste(proteins, collapse="|"),protein) & adj.pvalue < .05) %>% nrow()#68
FP<-m%>%filter(!grepl(paste(proteins, collapse="|"),protein) & adj.pvalue < .05) %>% nrow()#17
