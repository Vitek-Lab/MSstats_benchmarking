...
# Reading data
##############################
raw <- read.csv("C:/Users/tushi/OneDrive/Desktop/Data/DDA_CM_Progenesis/ControlMixture_DDA_Progenesis_input.csv", stringsAsFactors=F) # the data file
annot <- read.csv('C:/Users/tushi/OneDrive/Desktop/Data/DDA_CM_Progenesis/ControlMixture_DDA_Progenesis_annotation.csv')
...
TP<-test.MSstats %>% filter(grepl(paste(proteins, collapse="|"),Protein) & adj.pvalue < .05) %>% nrow() #193
FP<-test.MSstats %>% filter(!grepl(paste(proteins, collapse="|"),Protein)& adj.pvalue < .05) %>% nrow() #50



#################################################################################################################

#Preprocessing for accessing raw data for other packages

raw <- read.csv("C:/Users/tushi/OneDrive/Desktop/Data/DDA_CM_Progenesis/ControlMixture_DDA_Progenesis_input.csv", stringsAsFactors=F,skip = 2) # the data file
annot <- read.csv('C:/Users/tushi/OneDrive/Desktop/Data/DDA_CM_Progenesis/ControlMixture_DDA_Progenesis_annotation.csv')
raw=raw[,1:36]
...
input_data = raw %>% select(Accession, Sequence, Charge, starts_with("X"))
...
input_data = input_data %>% group_by(Accession, Run, PSM) %>% summarise(Intensity=max(Intensity))
input_data = input_data %>% filter(Accession != "")
input_data = input_data %>% dplyr::rename(ProteinName=Accession)
...
pep.count <- as.data.frame(raw )%>% select(Accession, Sequence)
pep.count<-pep.count%>%group_by(Accession)%>%
  summarise(count=n_distinct(Sequence),.groups = 'drop')%>%
  as.data.frame()
# Minimum peptide count of some proteins can be 0
# add pseudocount 1 to all proteins
pep.count$count = pep.count$count+1
rownames(pep.count) = pep.count$Accession
pep.count = pep.count %>% select(-Accession)
...
#calculating TP , FP
TP<-iprg_deqms_results %>% filter(grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow()#194
FP<-iprg_deqms_results %>% filter(!grepl(paste(proteins, collapse="|"),gene) & adj.P.Val < .05) %>% nrow()#460
...
df.LFQ = input_data %>% select(Accession, Run, Intensity)
df.LFQ$PSM = paste(input_data$Sequence, input_data$Charge, sep="_")
df.LFQ = df.LFQ %>% filter(Accession != "")
df.LFQ = df.LFQ %>% dplyr::rename(ProteinName=Accession)
...
TP<-iprg_msqrob_results %>% filter(grepl(paste(proteins, collapse="|"),Protein) & adjPval < .05) %>% nrow() #181
FP<-iprg_msqrob_results %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & adjPval < .05) %>% nrow() #134
...
e_mdata<-input_data[,c('Sequence','Run','Intensity')]
e_mdata$Run<-str_split_fixed(e_mdata$Run,"X",2)[,2]
e_mdata<-e_mdata%>%group_by(Sequence,Run)%>% summarise(Intensity=sum(Intensity))
...
#e_mdata<-subset(e_mdata, select = -c(PeptideSequence) )
...
mystats <- imd_anova(omicsData = myprodata, comparisons = comparisons,
                     test_method = "anova",pval_adjust_g="bonferroni")
...
TP<-m %>% filter(grepl(paste(proteins, collapse="|"),Protein) & `p-value` < .05) %>% nrow() #201
FP<-m %>% filter(!grepl(paste(proteins, collapse="|"),Protein) & `p-value` < .05) %>% nrow() #938
...
df.LFQ = as.data.frame(input_data) %>% select(Accession, Run, Intensity)
df.LFQ$Run<-str_split_fixed(df.LFQ$Run,"X",2)[,2]
df.LFQ = df.LFQ %>% group_by(Accession, Run) %>% summarise(Intensity = max(Intensity, na.rm = TRUE))
df.LFQ = df.LFQ %>% dplyr::rename(ProteinName=Accession)
df.LFQ = as.data.frame(df.LFQ)
...
TP<-df %>% filter(grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow() #203
FP<-df %>% filter(!grepl(paste(proteins, collapse="|"),ID) & p.adj < .05) %>% nrow() #470
...
TP<-test %>% filter(grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #194
FP<-test %>% filter(!grepl(paste(proteins, collapse="|"),name) & adj_pval < .05) %>% nrow() #417

######################################################################################################

###############################################prolfqua#######################################################

#Preprocessing for prolfqua
...
df.LFQ$Run<-substr(df.LFQ$Run,2,44)
...
TP<- m%>%filter(grepl(paste(proteins, collapse="|"),protein_Id) & `p-value` < .05) %>% nrow()#211
FP<-m%>%filter(!grepl(paste(proteins, collapse="|"),protein_Id) & `p-value` < .05) %>% nrow()#398
...
TP<- m%>%filter(grepl(paste(proteins, collapse="|"),protein) & adj.pvalue < .05) %>% nrow()#193
FP<-m%>%filter(!grepl(paste(proteins, collapse="|"),protein) & adj.pvalue < .05) %>% nrow()#394
